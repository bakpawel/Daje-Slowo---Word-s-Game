...
class RankingStorage {
    ...
    getRank(){
        if(localStorage.getItem("rank")){
            ...
            return [{points: 0, name: "..."},{points: 0, name: "..."},{points: 0, name: "..."}];
        }
    };
    ...
    saveScore(){
        ...
        this.rank.sort((a,b) => (a.points < b.points) ? 1 : ((b.points < a.points) ? -1 : 0))
        this.rank.pop();
...
class StartPage extends RenderComponent {
    ...
    rankingStorage(){
            ...
            console.log(rank);
            let renderTable = new RenderTableElements(document.querySelector('table'),['Ranking'],rank);
...
class CreateBoardButtons extends RenderComponent {
    ...
    constructor(hookId){
        ...
        this.gamelogic = new GameLogic(['po','ta','to','ma','mam','mu','za','do']);
    }
    
    //losowanie litery 
    
    drawLetter(){
        const Pl = ["A", "Ą", "B", "C", "Ć", "D", "E", "Ę", "F", "G", "H", "I", "J", "K", "L", "Ł", "M", "N", "Ń", "O", "Ó", "P", "R", "S", "Ś", "T", "U", "W", "Y", "Z", "Ź", "Ż"];
    ...
    createdRows = [this.renderSingleRow(),this.renderSingleRow(),this.renderSingleRow()];
...
class GameLogic extends RenderComponent{
    constructor(listOfWords){
        ...
        this.guessedWord = '';
        this.score = 0;
        console.log(this.rightWords, this.ammountOfWords, this.guessedWord, this.score);
    }
    ...
    checkWord(){
        const isCorrectWord = this.rightWords.includes(this.guessedWord.toLocaleLowerCase());
    ...
    removeWord(word){
        this.rightWords = this.rightWords.filter(element => element !=word.toLocaleLowerCase());
        console.log(this.rightWords);
    }

    renderWordOnList(){
        const listElement = this.createRootElement('li','',false);
